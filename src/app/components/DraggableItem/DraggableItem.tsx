...
export default function DraggableItem({ config, index }: DraggableItemProps) {
  ...
  const { setCurrentDraggingIndex, resetCurrentDraggingIndex } = useDraggingIndexSetters();
  ...
  const handleDrag = useCallback((event: DragEvent<HTMLDivElement>) => {
    ...
    if (event.clientX || event.clientY) { // prevent flash of resetting position to (0, 0)
  ...
  return (
    <>
      <div className="flex flex-row gap-8 items-center" draggable onDragStart={handleDragStart} onDrag={handleDrag} onDragEnd={handleDragEnd}>
        <CoverFitImage height={96} width={96} src={config.imageData} alt={config.imageAlt} />
      ...
      {/* Custom drag image */}
      <div className={clsx('flex flex-row gap-2 fixed z-20 pointer-events-none shadow-lg', {hidden: !isDragging})} style={{top: dragY, left: dragX}}>
        <CoverFitImage height={32} width={32} src={config.imageData} alt={config.imageAlt} />
        <div>{config.title}</div>
      </div>
