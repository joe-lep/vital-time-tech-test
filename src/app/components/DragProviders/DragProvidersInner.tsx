...
export default function DragProvidersInner({ children }: DragProvidersInnerProps) {
  const [draggingIndex, setDraggingIndex] = useState<number | null>(null);
  const [activeConfig, setActiveConfig] = useState<DraggableItemConfig | null>(null);
  ...
  return (
    <DndContext onDragStart={handleDragStart} onDragEnd={handleDragEnd} onDragCancel={handleDragCancel}>
      ...
      <DragOverlay className="flex items-center justify-center">
        {activeConfig != null && (
          <OverlayItem config={activeConfig} />
        )}
      </DragOverlay>
    </DndContext>
  );
